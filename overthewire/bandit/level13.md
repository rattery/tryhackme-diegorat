...
We will notice that the command will simply be output to the terminal. In this instance, we actually want it in a file since we know that the hexdump was compressed so we can begin decompressing. We will do this by piping/redirecting the output to a seperate file. 

`xxd -r data.txt > data1.txt`

Next we must repeatadly use the `file` command to observe the file's properties:
...
Letting us know that we now must up the `gzip` command to begin decompressing. This is done with the `-d` parameter to sign decompression and the `-S` paramater to sign the filename extension:

`gzip -d -S .txt data1.txt`

This will turn the `data1.txt` file into a `data1` file which when file is used on, will output:

`data1: bzip2 compressed data, block size = 900k`

Once again, signing us that now we must use the `bzip2` or `bzcat` command:
...
Use the `zcat` command since the `data2` file doesnt have a filename extension:
...
Netting us `data5.bin` in which is again a `POSIX tar archive (GNU)`, so we use the same command:
...
In which finally, we can `cat` the `data8` file, resulting in the output:
